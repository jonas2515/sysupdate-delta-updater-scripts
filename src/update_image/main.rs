...
fn help() {
    println!(
        "usage: update_image <old_image> <image_to_update> <new_image>
...
fn delta_update_image(
    ...
    let old_image_hashes_to_blocks = if let Ok(old_image_manifest) =
        delta_manifest::read_manifest(&format!("{old_image}.manifest"))
    {
    ...
    let new_image_block_hashes = if let Ok(new_image_manifest) =
        delta_manifest::read_manifest(&format!("{new_image}.manifest"))
    {
    ...
    println!(
        "Amount to copy from new image: {} MiB",
