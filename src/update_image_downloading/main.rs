...
fn help() {
    println!("usage: update_image_downloading <old_image> <image_to_update> <new_image_url>
...
async fn download_blocks_multipart(client: &Client, mut target: &File, downloads: &[(usize, u64)], url: &str) -> Result<(), Box<dyn Error>> {
    ...
    if content_type_header.is_some_and(|h| h.contains("multipart/byteranges")) {
        let boundary = content_type_header.unwrap().split("boundary=").nth(1)
        ...
        while let Some((offset, _n_chunks)) = d_iter.next() {
            let Some(Ok(part)) = m.next().await else { break };

            target.seek(std::io::SeekFrom::Start(*offset as u64 * delta_manifest::BLOCK_SIZE as u64))?;
    ...
    } else if content_type_header.is_some_and(|h| h == "application/octet-stream") || content_type_header.is_none() {
...
fn update_image(source_image: &str, image_to_update: &str, new_image_url: &str, blocks_to_update: Vec<u64>) -> Result<(), Box<dyn Error>> {
...
*/
    ...
    while i < downloads.len() {
        ...
        let future = download_blocks_multipart(&client, &image_to_update_file, &downloads[i..end_cur_range], new_image_url);
        block_on(future)?;
...
async fn download_manifest(new_image_manifest_url: &str) -> Result<delta_manifest::Manifest, Box<dyn Error>> {
...
fn create_verity_image(new_image: &str, new_verity_image: &str, verity_salt: [u8; 32]) -> Result<[u8; 32], Box<dyn Error>> {
    ...
    let root_hash = out_str.lines().nth(9).expect("not a line").split("\t").nth(1).expect("not a str");
    let root_as_vec = (*decode_hex(root_hash)?).try_into()?;

    Ok(root_as_vec)
}

fn delta_update_image(old_image: &str, image_to_update: &str, new_image_url: &str, new_image_manifest_url: &str) -> Result<(), Box<dyn Error>> {
    let old_image_manifest_filename = format!("{old_image}.manifest");
    let old_image_hashes_to_blocks = if let Ok(old_image_manifest) = delta_manifest::read_manifest(&old_image_manifest_filename) {
        println!("Found old image manifest at \"{}\"", old_image_manifest_filename);
    ...
    println!("Download size: {} MiB", ((new_blocks_to_old_blocks.len() - n_blocks_avail_in_old_image) * delta_manifest::BLOCK_SIZE) / 1024 / 1024);

    update_image(old_image, image_to_update, new_image_url, new_blocks_to_old_blocks)?;
    ...
    println!("Now creating new verity image: \"{}\"", verity_image_filename);
    let root_hash = create_verity_image(image_to_update, verity_image_filename, new_image_manifest.verity_salt)?;
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    match args.len() {
        5 => {
            ...
            delta_update_image(old_image, image_to_update, new_image_url, new_image_manifest_url)
        },
        _ => {
            help();
            process::exit(1)
